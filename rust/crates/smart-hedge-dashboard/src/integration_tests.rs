...
fn start_test_server(name: &str) -> Option<u16> {
    ...
    let dir = std::env::temp_dir().join(format!(
        "smart-hedge-dashboard-itest-{name}-{}-{n}",
        std::process::id()
    ));
    ...
    let env = EnvOverrides {
        core_binary: Some(core_binary.to_string_lossy().into_owned()),
...
fn health_endpoint_returns_200_and_reports_no_order_endpoint() {
    let Some(port) = start_test_server("health") else {
        return;
    };
...
fn index_page_returns_the_html_console() {
    let Some(port) = start_test_server("index") else {
        return;
    };
    let response = raw_request(
        port,
    ...
    assert!(
        response.contains("Smart Dynamic Hedge"),
        "response was: {response}"
    );
}
...
fn recommendation_endpoint_returns_a_paper_only_decision() {
    let Some(port) = start_test_server("recommendation") else {
        return;
    };
    let response = raw_request(
        port,
    ...
    assert!(
        response.contains("\"mode\":\"paper\""),
        "response was: {response}"
    );
    assert!(
        response.contains("\"live_execution_allowed\":false"),
        "response was: {response}"
    );
}
...
fn an_invalid_symbol_is_rejected_with_422() {
    let Some(port) = start_test_server("badsymbol") else {
        return;
    };
    let response = raw_request(
        port,
        "GET /api/recommendation?symbol=bad%20symbol HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
    );
    assert!(
...
fn replay_of_an_unknown_decision_returns_404() {
    let Some(port) = start_test_server("replay404") else {
        return;
    };
    let response = raw_request(
        port,
        "GET /api/replay/does-not-exist HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
    );
    assert!(
...
fn an_unknown_route_returns_404() {
    let Some(port) = start_test_server("unknownroute") else {
        return;
    };
    let response = raw_request(
        port,
...
fn a_non_get_method_returns_405() {
    let Some(port) = start_test_server("wrongmethod") else {
        return;
    };
    let response = raw_request(
        port,
...
fn history_endpoint_returns_a_json_array() {
    let Some(port) = start_test_server("history") else {
        return;
    };
    let response = raw_request(
        port,
    ...
    assert!(
        response[body_start..].trim_start().starts_with('['),
        "response was: {response}"
    );
}
...
fn recommendation_then_history_shows_the_new_decision() {
    let Some(port) = start_test_server("recthenhist") else {
        return;
    };
    let rec = raw_request(
        port,
    ...
    let hist = raw_request(
        port,
        "GET /api/history?symbol=SPY HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
    );
