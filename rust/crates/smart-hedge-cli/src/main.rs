...
fn run(raw_args: &[String]) -> Result<i32, CliError> {
    ...
    match parsed.command {
        ...
        Command::Once {
            ...
            overrides,
            model,
        } => commands::cmd_once(parsed.config, &symbol, overrides, model),
        Command::Loop {
            symbol,
            overrides,
            interval,
            model,
        } => commands::cmd_loop(parsed.config, &symbol, overrides, interval, model),
        Command::Replay { decision_id } => commands::cmd_replay(parsed.config, &decision_id),
        Command::Recent { limit, symbol } => {
            commands::cmd_recent(parsed.config, limit, symbol.as_deref())
        }
        ...
        Command::Backtest {
            ...
            days,
            start,
        } => commands::cmd_backtest(parsed.config, &symbol, days, start),
        Command::Autonomous {
            symbol,
            overrides,
            interval,
