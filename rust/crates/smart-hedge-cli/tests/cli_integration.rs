...
fn prebuilt_core_binary(root: &Path) -> Option<PathBuf> {
    let direct = root.join("build").join(if cfg!(windows) {
        "smart_dynamic_hedge.exe"
    } else {
        "smart_dynamic_hedge"
    });
    if direct.is_file() {
...
fn harness_or_skip(name: &str) -> Option<Harness> {
    ...
    let Some(core_binary) = prebuilt_core_binary(&root) else {
        eprintln!(
            "skipping {name}: no prebuilt core binary found under {}/build",
            root.display()
        );
        return None;
    };
    ...
    let dir =
        std::env::temp_dir().join(format!("smart-hedge-cli-itest-{}-{n}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    Some(Harness {
        root,
        core_binary,
        db_path: dir.join("decisions.sqlite3"),
    })
}
...
fn serve_starts_a_real_http_server_and_answers_health() {
    let Some(h) = harness_or_skip("serve_starts_a_real_http_server_and_answers_health") else {
        return;
    };
    ...
    assert!(
        response.contains("\"broker_order_endpoint_present\":false"),
        "response was: {response}"
    );
...
fn mcp_answers_initialize_and_tools_list_over_stdio() {
    let Some(h) = harness_or_skip("mcp_answers_initialize_and_tools_list_over_stdio") else {
        return;
    };
    ...
    let init_response: serde_json::Value =
        serde_json::from_str(&init_line).expect("stdout line should be JSON");
    assert_eq!(
    ...
    let tools_response: serde_json::Value =
        serde_json::from_str(&tools_line).expect("stdout line should be JSON");
...
fn once_produces_a_paper_only_decision_as_pretty_json() {
    let Some(h) = harness_or_skip("once_produces_a_paper_only_decision_as_pretty_json") else {
        return;
    };
...
fn recent_and_replay_see_a_decision_persisted_by_a_prior_process() {
    let Some(h) = harness_or_skip("recent_and_replay_see_a_decision_persisted_by_a_prior_process")
    else {
        return;
    };
    ...
    assert!(
        once_output.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&once_output.stderr)
    );
    let decision: serde_json::Value =
        serde_json::from_str(&String::from_utf8_lossy(&once_output.stdout)).unwrap();
    ...
    let recent: serde_json::Value =
        serde_json::from_str(&String::from_utf8_lossy(&recent_output.stdout)).unwrap();
    assert!(
        recent
            .as_array()
            .unwrap()
    ...
    let replayed: serde_json::Value =
        serde_json::from_str(&String::from_utf8_lossy(&replay_output.stdout)).unwrap();
...
fn replay_of_an_unknown_decision_id_exits_2() {
    let Some(h) = harness_or_skip("replay_of_an_unknown_decision_id_exits_2") else {
        return;
    };
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("decision not found"),
        "stderr was: {stderr}"
    );
    h.cleanup();
}
...
fn self_test_passes_against_the_synthetic_heuristic_path() {
    let Some(h) = harness_or_skip("self_test_passes_against_the_synthetic_heuristic_path") else {
        return;
    };
