...
pub fn build_advisor_from_uri(
    loaded: &LoadedConfig,
    uri: &ModelUri,
) -> Result<Box<dyn Advisor>, AdvisorError> {
    match uri.scheme.as_str() {
        "heuristic" | "none" | "local" => Ok(Box::new(HeuristicAdvisor)),
        "openai" | "responses" => {
            let api_key = std::env::var("OPENAI_API_KEY").unwrap_or_default();
            let advisor =
                OpenAIAdvisor::with_explicit_model(loaded, api_key, uri.identifier.clone())?;
            Ok(Box::new(advisor))
        }
        other => Err(AdvisorError(format!(
            "unknown model URI scheme {other:?} (expected heuristic:// or openai://)"
        ))),
    }
}
...
mod tests {
    ...
    fn loaded_config() -> LoadedConfig {
        ...
        let dir = std::env::temp_dir().join(format!(
            "smart-hedge-model-advisor-router-test-{}-{n}",
            std::process::id()
        ));
