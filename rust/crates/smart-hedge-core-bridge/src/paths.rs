...
mod tests {
    ...
    fn windows_fallback_path_is_under_release_subdirectory() {
        ...
        assert_eq!(
            path,
            PathBuf::from("/root/build/Release/smart_dynamic_hedge.exe")
        );
    }
    ...
    fn loaded_config_with_core_binary(core_json: &str) -> LoadedConfig {
        ...
        let dir = std::env::temp_dir().join(format!(
            "smart-hedge-core-bridge-resolve-binary-test-{}-{n}",
            std::process::id()
        ));
    ...
    fn resolve_binary_with_no_override_falls_back_to_the_platform_default() {
        ...
        assert_eq!(
            resolved,
            default_binary_path(Path::new("/some/other/project/root"))
        );
    }
    ...
    fn resolve_binary_with_an_explicit_absolute_path_uses_it_verbatim() {
        ...
        let absolute = if cfg!(windows) {
            r"C:\opt\custom\smart_dynamic_hedge"
        } else {
            "/opt/custom/smart_dynamic_hedge"
        };
        let loaded = loaded_config_with_core_binary(&format!(
            r#"{{"binary": "{}"}}"#,
            absolute.replace('\\', "\\\\")
        ));
        let resolved = resolve_binary(&loaded, Path::new("/some/other/project/root"));
    ...
    fn resolve_binary_with_an_explicit_relative_path_resolves_against_config_dir_not_project_root()
    {
        ...
        assert_eq!(
            resolved,
            loaded.config_dir.join("bin").join("smart_dynamic_hedge")
        );
