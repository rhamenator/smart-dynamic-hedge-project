...
mod tests {
    ...
    fn a_response_within_the_cap_is_read_in_full() {
        let port = crate::mock_http_test_support::start(vec![(
            "/x",
            (200, "text/plain", "hello".to_string()),
        )]);
        let response = ureq::get(&format!("http://127.0.0.1:{port}/x"))
            .call()
            .unwrap();
    ...
    fn a_response_larger_than_the_cap_is_truncated_not_unbounded() {
        ...
        let port =
            crate::mock_http_test_support::start(vec![("/x", (200, "text/plain", large_body))]);
        let response = ureq::get(&format!("http://127.0.0.1:{port}/x"))
            .call()
            .unwrap();
        ...
        assert_eq!(
            body.len(),
            100,
            "expected the read to stop at the cap, not read the full 10,000 bytes"
        );
    }
}
